...
  const navigate = useNavigate();
  ...
  // Đã hợp nhất: Dùng cấu trúc object { label, slug }
  const menuItems = [
...
              <input className="hidden md:block border rounded-full px-4 py-2 text-sm w-64" placeholder="Tìm kiếm" />
              {isLoggedIn ? (
...
              {menuItems.map((item) => (
                <li key={item.slug}>
                  <Link to={`/${item.slug}`} className="px-2 py-2 text-xs font-semibold uppercase hover:bg-blue-700 block">
                    {item.label}
                  </Link>
                </li>
              ))}
...
                      <Link to={`/${parentSlug}`}>{group.title}</Link>
                    </h4>
...
                  <div
                    key={item.label}
                    onClick={() => navigate(`/format/${item.label.toLowerCase()}`)}
...
                <li className="flex items-center gap-3"><LightBulbIcon className="w-5 h-5" /> Lý tưởng sống</li>
                <li className="flex items-center gap-3"><MegaphoneIcon className="w-5 h-5" /> Nói thẳng</li>
                <li className="flex items-center gap-3"><StarIcon className="w-5 h-5" /> Tin độc quyền</li>
                <li className="flex items-center gap-3"><GlobeAltIcon className="w-5 h-5" /> Thị trường</li>
                <li className="flex items-center gap-3"><EnvelopeIcon className="w-5 h-5" /> Liên hệ</li>
                <li className="flex items-center gap-3"><BuildingOffice2Icon className="w-5 h-5" /> Thông tin tòa soạn</li>
                <li className="flex items-center gap-3"><RectangleGroupIcon className="w-5 h-5" /> Liên hệ quảng cáo</li>
              </ul>
